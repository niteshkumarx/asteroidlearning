...
$gtx_query.=" WHERE city='".$_SESSION['city']."' and course='".$_SESSION['course']."' and gtx_email!='".$_SESSION['email']."' and gtx_asteroid='1' and gtx_email not in (select reciever_email_id from pre_invitations where senders_email_id='".$_SESSION['email']."' and status='ACCEPTED' ) order by RAND()";
$gtx_result=mysql_query($gtx_query,$gtx_connection);

if(mysql_num_rows($gtx_result)==0 )
{ 
echo "Oops! It seems there are no learners in your city with the same interest as you, try searching everyone.";
